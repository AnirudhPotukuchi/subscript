...
exports.getMetrics = exports.resolveModeration = exports.getModerationQueue = void 0;
const db_1 = require("../services/db");
const constants_1 = require("../constants");
const getModerationQueue = async (req, res) => {
    try {
        const flaggedPosts = await db_1.dbService.posts.findMany((p) => p.moderationStatus === 'flagged_hidden' || p.moderationStatus === 'pending_review');
        // We can fetch comments under moderation too
        const flaggedComments = await db_1.dbService.comments.findMany((c) => c.moderationStatus === 'flagged_hidden' || c.moderationStatus === 'pending_review');
...
const resolveModeration = async (req, res) => {
    ...
    try {
        ...
        if (targetType === 'Post') {
            ...
            if (!post)
                return res.status(constants_1.HTTP_STATUS.NOT_FOUND).json({ success: false, message: 'Post not found.' });
            await db_1.dbService.posts.update(id, { moderationStatus: statusValue });
        }
        else if (targetType === 'Comment') {
            const comment = await db_1.dbService.comments.get(id);
            if (!comment)
                return res.status(constants_1.HTTP_STATUS.NOT_FOUND).json({ success: false, message: 'Comment not found.' });
            await db_1.dbService.comments.update(id, { moderationStatus: statusValue });
        }
        else {
            return res.status(constants_1.HTTP_STATUS.BAD_REQUEST).json({
                success: false,
        ...
        // Add Audit Log
        const log = await db_1.dbService.auditLogs.add({
...
const getMetrics = async (req, res) => {
    try {
        const allUsers = await db_1.dbService.users.findMany();
        const allPosts = await db_1.dbService.posts.findMany();
        const allInterviews = await db_1.dbService.interviews.findMany();
        const allResources = await db_1.dbService.resources.findMany();
