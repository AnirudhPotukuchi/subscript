...
export const getProfile = async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  try {
    const user = await dbService.users.get(id);
    if (!user) {
    ...
    const { password, verificationToken, resetPasswordToken, resetPasswordExpires, ...safeProfile } = user as any;
...
export const updateProfile = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const existing = await dbService.users.get(uid);
    if (!existing) {
      return res.status(HTTP_STATUS.NOT_FOUND).json({
        success: false,
    ...
    const updatedProfile = await dbService.users.update(uid, {
...
export const followUser = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const currentUser = await dbService.users.get(currentUid);
    const targetUser = await dbService.users.get(targetUid);
    ...
    await dbService.users.update(currentUid, {
      following: updatedFollowing,
      followingCount: updatedFollowing.length
    });

    await dbService.users.update(targetUid, {
